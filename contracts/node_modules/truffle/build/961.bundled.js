...
const Box = {
    unbox: async (url, destination, options = {}, config) => {
        ...
        try {
            ...
            await utils_1.default.downloadBox(normalizedSourcePath, tempDirPath, events);
            const boxConfig = await utils_1.default.readBoxConfig(tempDirPath);
            await utils_1.default.unpackBox(tempDirPath, destination, boxConfig, unpackBoxOptions);
    ...
    checkDir: async (options = {}, destination) => {
        ...
        if (!options.force) {
            ...
            if (unboxDir.length) {
                ...
                const answer = await inquirer_1.default.prompt(prompt);
                if (!answer.proceed) {
...
const path_1 = __importDefault(__webpack_require__(385622));
module.exports = {
    downloadBox: async (source, destination, events) => {
        events.emit("unbox:downloadingBox:start");
        try {
            await unbox_1.default.verifySourcePath(source);
            await unbox_1.default.fetchRepository(source, destination);
    ...
    readBoxConfig: async (destination) => {
        ...
        return await config_1.default.read(configPath);
    },
    ...
    unpackBox: async (tempDir, destination, boxConfig, unpackBoxOptions) => {
        ...
        await unbox_1.default.copyTempIntoDestination(tempDir, destination, unpackBoxOptions);
    },
...
async function verifyVCSURL(url) {
    ...
    try {
        await axios_1.default.head(repoUrl, { maxRedirects: 50 });
    }
    catch (error) {
...
async function verifySourcePath(sourcePath) {
    if (sourcePath.startsWith("/")) {
        return verifyLocalPath(sourcePath);
    }
    return verifyVCSURL(sourcePath);
}
async function gitIgnoreFilter(sourcePath) {
    const ignoreFilter = ignore_1.default();
    try {
        const gitIgnore = await fs_extra_1.default.readFile(path_1.default.join(sourcePath, ".gitignore"), "utf8");
        ignoreFilter.add(gitIgnore.split(/\r?\n/).map(p => p.replace(/\/$/, "")));
    }
    catch (err) { }
    return ignoreFilter;
}
async function fetchRepository(sourcePath, dir) {
    if (sourcePath.startsWith("/")) {
...
async function promptOverwrites(contentCollisions, logger = console) {
    ...
    for (const file of contentCollisions) {
        ...
        const { overwrite } = await inquirer_1.default.prompt(overwriting);
        if (overwrite) {
...
const TruffleError = __webpack_require__(673321);
async function* preserve(options) {
    ...
    for (const recipe of plan) {
        ...
        const results = yield* control_1.control({ name, method }, { inputs, settings });
